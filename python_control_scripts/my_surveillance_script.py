...
class CustomController:
    ...
    def position_control(self, positions, vel_lin, vel_ang, tour_num):
        ...
        while not rospy.is_shutdown():#TODO: get rid of this while loop we only want to traverse wp's once!
            ...
            if self.current_state.armed and self.current_state.mode == "OFFBOARD":
                ...
                for i in range(len(positions)* tour_num):
                    index = i % len(positions)
                    self.reach_position(positions[index][0], positions[index][1], positions[index][2], vel_lin, vel_ang)
...
def parsePositions():
    ...
    Sample call:
        $python2.7 my_mission_script.py --positions 10 10 10 50 50 20 50 -50 20 -50 -50 20 0 0 20
    Sample return:
    ...
    CLI.add_argument(
    "--tour_num",  # name on the CLI - drop the `--` for positional/required parameters
    type=int,
    required=True,
    )
    ...
    return pos, args.linear_velocity, args.angular_velocity, args.tour_num
...
if __name__ == '__main__':
    """
    Sample call:
        $python2.7 my_surveillance_script.py --positions 10 10 10 50 50 20 50 -50 20 -50 -50 20 0 0 20 --linear_velocity 5 --angular_velocity 5 --tour_num 2

    this flies drone to coordinates in [(10.0, 10.0, 10.0), (50.0, 50.0, 20.0), (50.0, -50.0, 20.0), (-50.0, -50.0, 20.0), (0.0, 0.0, 20.0)] in OFFBOARD mode
    with linear velocity of 5 and angular velocity of 5 and takes this route for 2 times.
    """
    try:
        positions, vel_lin, vel_ang, tour_num = parsePositions()
        customController = CustomController()
        customController.checkForServices()
        customController.checkForTopics()
        customController.checkForFCU()
        customController.position_control(positions, vel_lin, vel_ang, tour_num)
